...
export default function PageNavPill({ items }: PageNavPillProps) {
  return (
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between w-full gap-6">
      <TransitionLink
        href="#serveis-list"
        className="group flex items-center gap-2 text-body-l lg:text-body-xl font-medium hover:opacity-70 transition-opacity"
      >
      ...
      <nav className="flex items-center gap-2 p-1.5 bg-border-subtle/30 rounded-full w-fit">
        {items.map((item, index) => (
          <TransitionLink
            key={index}
            href={item.href}
            className={`px-6 py-2 rounded-full text-body-s md:text-body-m lg:text-body-l transition-all duration-300 ${index === 0
